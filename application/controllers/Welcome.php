...
class Welcome extends CI_Controller
{
	...
	public function index()
	{
		...
		$total_records = $this->menu->get_total();
		if (!empty($kw)) {
			$total_records = $this->menu->get_total_search($kw);
		}
		if ($total_records > 0) {
			//get current page records
			$data["records"] = $this->menu->pagination($limit_per_page, $start_index);
			if (!empty($kw)) {
				$data["records"] = $this->menu->search($kw, $limit_per_page, $start_index);
			}
